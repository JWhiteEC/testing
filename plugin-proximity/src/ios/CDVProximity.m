...
NSLog(@"Proximity: started");
}

- (void)onReset
{
NSLog(@"Proximity: reset");
    [self stop:nil];
}
...
NSLog(@"Proximity: stopped");
}

- (void)sensorStateChange:(NSNotificationCenter *)notification
{
NSLog(@"Proximity: state changed from %d", status);
    status = [[UIDevice currentDevice] proximityState];
NSLog(@"Proximity: state changed to %d", status);
}
